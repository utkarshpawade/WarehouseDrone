...
class WareHouseClient(Node):
    ...
    #     """
    #     if len(msg.data) >= 6 and not self.done:
    #         self.get_logger().info('Received random points for path planning.')
            # x_one, y_one, x_two, y_two, x_three, y_three  = msg.data[:6]
            # self.one_point.position.x = float(x_one)
            # self.one_point.position.y = float(y_one)
            # self.one_point.position.z = 27.0
            # self.two_point.position.x = float(x_two)
            # self.two_point.position.y = float(y_two)
            # self.two_point.position.z = 27.0
            # self.three_point.position.x = float(x_three)
            # self.three_point.position.y = float(y_three)
