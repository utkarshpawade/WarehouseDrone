...
class PathPlanningServer(Node):
    ...
    def __init__(self):
        ...
        self.moving_avg_window = 20  # Number of samples for the moving average
        ...
        # self.Kp = [5, 5, 14, 0]
        # self.Ki = [0.0, 0.0, 0.07, 0.0]
        # self.Kd = [200,200,312,0]
        self.Kp = [5, 5, 9, 0]
        self.Ki = [0.0, 0.0, 0.02, 0.0]
        self.Kd = [150,150,250,0]
    ...
    def listener_callback(self,msg):
        if len(msg.data) >= 2:
            ...
            self.goal_one = pixel_to_whycon(x_start,y_start)

            x_end = msg.data[2]
            y_end = msg.data[3]

            self.goal_two = pixel_to_whycon(x_end,y_end)

            # Set the start and end points
    
                # First point is start_point
            self.start_point = Pose()
            self.start_point.position.x = self.goal_one[0]
            self.start_point.position.y = self.goal_one[1]
            self.start_point.position.z = self.goal_one[2]  
            ...
            self.end_point = Pose()
            self.end_point.position.x = self.goal_two[0]
            self.end_point.position.y = self.goal_two[1]
            self.end_point.position.z = self.goal_two[2] 
    ...
    def execute_callback(self, goal_handle):
        ...
        if (self.setpoint == self.goal_one) or (self.setpoint == self.goal_two):
            self.get_logger().error('The SetPoint Will Hover Now ---------------')
            self.hover = True
        ...
        while True:
            ...
            #     if self.max_time_inside_sphere >= 0:
            #         break
...
def main(args=None):
    ...
    waypoint_server = PathPlanningServer()
